...
if (
  ...
  $config['environment_indicator.indicator']['bg_color'] = '#005cb9';
  // Set logging level on production.
  $config['system.logging']['error_level'] = 'verbose';
  // Disable GTM
  $config['google_tag.container.primary']['container_id'] = '';
...
if (
  ...
  // Environment Indicator settings
  $config['environment_indicator.indicator']['name'] = 'Pantheon Prod [Master DB]';
  $config['environment_indicator.indicator']['bg_color'] = '#b90000';
  // Set logging level on production.
  $config['system.logging']['error_level'] = 'hide';
  // Set GTM Code
  $config['google_tag.container.primary']['container_id'] = 'GTM-M4MHC6S';
  // Enable CSS and JS aggregation
  $config['system.performance']['css']['preprocess'] = TRUE;
  $config['system.performance']['js']['preprocess'] = TRUE;
  $config['advagg.settings']['enabled'] = TRUE;
  // Only allow search indexing on live
  $config['search_api.index.opensolr_sitewide_index']['read_only'] = FALSE;
}
 ...
 */
if (isset($_ENV['PANTHEON_ENVIRONMENT'])) {
  $settings['file_temporary_path'] = $_SERVER['HOME'] .'/tmp';
}
...
/**
 * Redirects
 * Pantheon Docs: https://pantheon.io/docs/redirects
 */
...
if (isset($_ENV['PANTHEON_ENVIRONMENT'])) {
  ...
  $config['search_api.index.content']['server'] = 'pantheon';
  $config['search_api.content']['read_only'] = FALSE;
}

/**
 * Fast 404
 *
 * Drupal core has an option to serve a simple static HTML error response
 * to a 404 status code. Enabling this setting can decrease wasted bandwidth
 * and server load.
 *
 * Reference article: https://support.acquia.com/hc/en-us/articles/360005246714-Using-Fast-404-to-improve-Drupal-performance
 */
if (isset($_ENV['PANTHEON_ENVIRONMENT'])) {
